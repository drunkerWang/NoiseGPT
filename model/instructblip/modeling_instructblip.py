...
class InstructBlipQFormerMultiHeadAttention(nn.Module):
    def __init__(self, config, is_cross_attention=False):
        ...
        # print('encoder_hidden_size', config.encoder_hidden_size)
    ...
    def forward(
        ...
        if pixel_values is None:
            if embeds is None:
                ...
                # sorted_input_ids, _ = torch.sort(input_ids, descending=True)
                # print('input_ids', input_ids.shape, input_ids, sorted_input_ids)
                inputs_embeds = self.get_input_embeddings()(input_ids)
                image_embeds_index = torch.where(input_ids == sp_token)
                # print('inputs_embeds', inputs_embeds[image_embeds_index].shape) 
                # print('language_model_inputs', language_model_inputs.reshape(-1,language_model_inputs.shape[-1]).shape)   
